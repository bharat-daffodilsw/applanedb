        ...
        ApplaneDB.connect(Config.URL, Config.DB, function (err, db) {
            db.query({$collection: "tasks"}, function (err, result) {
                ...
                db.batchUpdateById(update, function (err, data) {
                    ...
                    expect(ModuleManager.getSequence("update")).to.have.length(9);
                    ...
                    expect(ModuleManager.getSequence("update")[3].name).to.eql("DBRef");
                    expect(ModuleManager.getSequence("update")[4].name).to.eql("Function");
                    expect(ModuleManager.getSequence("update")[5].name).to.eql("DataType");
                    ...
                    expect(ModuleManager.getSequence("update")[8].name).to.eql("Child");
                    done();
                });
            });
        });
